...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setIDCCSchedule(idccDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = idccDGP.getBlockSize(nHeight);
    minGasPrice = idccDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = idccDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    IDCCTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractIDCCTX resultConverter;
    if(!convert.extractionIDCCTransactions(resultConverter)){
    ...
    for(IDCCTransaction idccTransaction : idccTransactions){
        txGas += idccTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + idccTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(idccTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, idccTransactions, hardBlockGasLimit);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet)
{
    // Found a solution (stake)
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("IDCCMiner : generated block is stale");

        for(const CTxIn& vin : pblock->vtx[1].vin) {
            if (wallet.IsSpent(vin.prevout.hash, vin.prevout.n)) {
                return error("IDCCMiner : Gen block stake is invalid - UTXO spent");
            }
        }
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("IDCCMiner : ProcessNewBlock, block not accepted");
    }
