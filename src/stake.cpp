...
static const int nIDCCProtocolSwitchHeightTestnet = 95150; // 2018-11-29 02:54:56 (1543460096)
...
// IDCC Stake modifier used to hash the stake kernel which is chosen as the stake
...
static bool GetIDCCStakeKernel(unsigned int nTimeTx, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake) {
...
// Note: Separated GetKernelStakeModifier & GetIDCCStakeKernel for convenient in case we need any other fork later on
static bool GetKernelStakeModifier(unsigned int nTimeTx, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake) {
    return GetIDCCStakeKernel(nTimeTx, nStakeModifier, nStakeModifierHeight, nStakeModifierTime, fPrintProofOfStake);
}
...
bool Stake::CheckHash(const CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int& nTimeTx, uint256& hashProofOfStake) {
    ...
    const int nNewPoSHeight = IsTestNet() ? nIDCCProtocolSwitchHeightTestnet : nIDCCProtocolSwitchHeight;
