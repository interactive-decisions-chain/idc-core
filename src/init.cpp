...
std::string HelpMessage(HelpMessageMode mode) {
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "idchain.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "idchaind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the IDC and zIDC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 31569, 42133));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in IDC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
                                   FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in IDC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, idchain, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in IDC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all IDC specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzidc=<n>", strprintf(_("Enable automatic wallet backups triggered after each zIDC minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-resetspentzerocoin", strprintf(_("*You can lose zIDC if you run this before the spend is confirmed on the blockchain.*  Reset current zIDC spends to match those on the blockchain.%s"), ""));
    strUsage += HelpMessageOpt("-resetmintzerocoin", strprintf(_("*You can lose zIDC if you run this before the mint is confirmed on the blockchain.*  Reset current zIDC mints to match those on the blockchain.%s"), ""));

//    strUsage += "  -anonymizeidchainamount=<n>     " + strprintf(_("Keep N IDC anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 40896, 42132));
...
std::string LicenseInfo() {
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The IDChain Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that IDChain is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. IDChain Core is shutting down."));
...
#endif
    // Make sure only a single IDChain process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. IDChain Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //IDChain specific: zerocoin and spork DB's
                ...
                // IDChain: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    ...
                    if (nZerocoinStartHeight != 0) {
                        if (chainActive.Height() > nZerocoinStartHeight) {
                            RecalculateZIDCMinted();
                            RecalculateZIDCSpent();
                        }
                    }
                    RecalculateIDCSupply(1);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of IDChain Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart IDChain Core to complete") << "\n";
        ...
        bool fEnableZIDCBackups = GetBoolArg("-backupzidc", true);
        pwalletMain->setZIDCAutoBackups(fEnableZIDCBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeIDChainAmount = GetArg("-anonymizeidchainamount", 0);
//    if (nAnonymizeIDChainAmount > 999999) nAnonymizeIDChainAmount = 999999;
//    if (nAnonymizeIDChainAmount < 2) nAnonymizeIDChainAmount = 2;
    ...
    LogPrintf("Anonymize IDChain Amount %d\n", nAnonymizeIDChainAmount);
    ...
    /* Denominations
       ...
       For example:
       1IDC+1000 == (.1IDC+100)*10
       10IDC+10000 == (1IDC+1000)*10
    */
