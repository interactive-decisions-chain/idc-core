...
#if defined(NDEBUG)
#error "IDCC cannot be compiled without assertions."
#endif
...
std::unique_ptr<IDCCState> globalState = nullptr;
...
std::string SCVersion ("/IDCCcore:5.2.5/");
...
unsigned int dgpMaxBlockSize = 2000000; // idcc
...
const string strMessageMagic = "IDCC Signed Message:\n";
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            IDCCDGP idccDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = idccDGP.getMinGasPrice(chainActive.Height() + 1);
            uint64_t blockGasLimit = idccDGP.getBlockGasLimit(chainActive.Height() + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            IDCCTxConverter converter(tx, NULL);
            ExtractIDCCTX resultConverter;
            if(!converter.extractionIDCCTransactions(resultConverter)){
            ...
            std::vector<IDCCTransaction> idccTransactions = resultConverter.first;
            std::vector<EthTransactionParams> idccETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(IDCCTransaction idccTransaction : idccTransactions){
                sumGas += idccTransaction.gas() * idccTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, idccTransaction.gasPrice());
                } else {
                    txMinGasPrice = idccTransaction.gasPrice();
                }
                VersionVM v = idccTransaction.getVersion();
                if(v.format!=0)
                ...
                if(idccTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(idccTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(idccTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)idccTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(idccETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > idccTransactions.size())
...
#if 0
    IDCCDGP idccDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setIDCCSchedule(idccDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//idccDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//idccDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//idccDGP.getBlockSize(pindex->nHeight + 1);
        ...
        if (fAddressIndex) { // OUTPUTS
            for (size_t k = 0; k < tx.vout.size(); k++) {
                ...
                if (ExtractDestination(out.scriptPubKey, dest, &txType)) {
                    ...
                    // outputs like OP_RETURN custom data, some IDCC v3 vouts had 1 satoshi in nValue
...
///////////////////////////////////////////////////////////////////////////////////////// idcc
        ...
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                IDCCTxConverter convert(tx, &view, &block.vtx);

                ExtractIDCCTX resultConvertIDCCTX;
                if(!convert.extractionIDCCTransactions(resultConvertIDCCTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertIDCCTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertIDCCTX.first, blockGasLimit);
                ...
                for(IDCCTransaction& ltx : resultConvertIDCCTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertIDCCTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(pindex->nHeight >= Params().SwitchPhi2Block()), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertIDCCTX.first[k].from(), resultConvertIDCCTX.first[k].to(),
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // Divide to keep a check precision of 0.01 IDCC
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, CNode* pfrom, const CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nHeight >= nIDCCProtocolSwitchHeight)
    {
 ...
 * As an example, IDCCcore uses /IDCCcore:X.X.X/ version string. So, this function will
 * return 5 for /IDCCcore:5.0.0/ or 0 if it couldn't find ":" or "." delimeters, or if no digits are between
 * those delimeters, or if string verison is empty
 * @param cleanVersion IDCCcore wallet version as a string
 * @return Major IDCCcore major version or 0
 */
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    IDCCTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<IDCCTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path idccDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(idccDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(idccDir.string(), std::ios::in | std::ios::out);
...
bool IDCCTxConverter::extractionIDCCTransactions(ExtractIDCCTX& idcctx){
    std::vector<IDCCTransaction> resultTX;
    ...
    idcctx = std::make_pair(resultTX, resultETP);
    return true;
}

bool IDCCTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool IDCCTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
IDCCTransaction IDCCTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    IDCCTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = IDCCTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = IDCCTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
