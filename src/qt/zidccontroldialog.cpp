...
#include "ui_zidccontroldialog.h"
...
std::list<std::string> ZIdcControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZIdcControlDialog::listMints;

ZIdcControlDialog::ZIdcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZIdcControlDialog),
    model(0) {
    ui->setupUi(this);
    listMints.clear();
...
ZIdcControlDialog::~ZIdcControlDialog() {
    delete ui;
}

void ZIdcControlDialog::setModel(WalletModel *model) {
    this->model = model;
    updateList();
}

//Update the tree widget
void ZIdcControlDialog::updateList() {
...
void ZIdcControlDialog::updateSelection(QTreeWidgetItem* item, int column) {
...
void ZIdcControlDialog::updateLabels() {
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZIdc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZIdcControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZIdcControlDialog::GetSelectedMints() {
...
void ZIdcControlDialog::ButtonAllClicked() {
    ui->treeWidget->blockSignals(true);
