...
AddEditIDCCNode::AddEditIDCCNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditIDCCNode)
{
    ui->setupUi(this);

}

AddEditIDCCNode::~AddEditIDCCNode()
{
    delete ui;
}


void AddEditIDCCNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMyIDCCNodes.insert(make_pair(c.sAddress, c));
	walletdb.WriteIDCCNodeConfig(c.sAddress, c);
        uiInterface.NotifyIDCCNodeChanged(c);

        accept();
    }
}

void AddEditIDCCNode::on_cancelButton_clicked()
{
    reject();
}
