...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasIDCCAddress = IsValidDestination(addr);
        if (hasIDCCAddress) {
            ui->label->setText("Address (IDCC)");
        ...
        isAddressValid = hasHexAddress || hasIDCCAddress;
    }
