...
PrivacyDialog::PrivacyDialog(QWidget* parent) : QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint | Qt::WindowCloseButtonHint),
    ...
    // "Spending 999999 zIDC ought to be enough for anybody." - Bill Gates, 2017
    ui->zIDCpayAmount->setValidator(new QDoubleValidator(0.0, 21000000.0, 20, this));
    ...
    ui->labelzIDCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_22_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzIDC->setEnabled(false);
        ui->pushButtonMintzIDC->setToolTip(tr("zIDC is currently disabled due to maintenance."));

        ui->pushButtonSpendzIDC->setEnabled(false);
        ui->pushButtonSpendzIDC->setToolTip(tr("zIDC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked() {
    ...
    if (dlg.exec()) {
        ...
        ui->zIDCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzIDC_clicked() {
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    if (GetAdjustedTime() < GetSporkValue(SPORK_21_ENABLE_ZEROCOIN)) {
        QMessageBox::information(this, tr("Mint Zerocoin"), tr("Zerocoin functionality is not enabled on the IDChain network yet."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }

    if(GetAdjustedTime() > GetSporkValue(SPORK_22_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"), tr("zIDC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zIDC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zIDC in ") +
...
void PrivacyDialog::on_pushButtonSpendzIDC_clicked() {

    if (!walletModel || !walletModel->getOptionsModel() || !pwalletMain)
        return;

    if (GetAdjustedTime() < GetSporkValue(SPORK_21_ENABLE_ZEROCOIN)) {
        QMessageBox::information(this, tr("Spend Zerocoin"), tr("Zerocoin functionality is not enabled on the IDChain network yet."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zIDC
        sendzIDC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zIDC
    sendzIDC();
}

void PrivacyDialog::on_pushButtonZIDCControl_clicked() {
    ZIdcControlDialog* zIdcControl = new ZIdcControlDialog(this);
    zIdcControl->setModel(walletModel);
    zIdcControl->exec();
}

void PrivacyDialog::setZIdcControlLabels(int64_t nAmount, int nQuantity) {
    ui->labelzIDCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzIDC() {
    QSettings settings;

    // Double is allowed now
    double dAmount = ui->zIDCpayAmount->text().toDouble();
    ...
    if(ui->payTo->text().isEmpty()) {
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid IDChain Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zIDC is requested
    ...
    if(!fWholeNumber && fMintChange) {
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " IDC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zIDC</b>";
    ...
    // use mints from zIDC selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZIdcControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZIdcControlDialog::GetSelectedMints();
    }
    ...
    // Clear zidc selector in case it was used
    ZIdcControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zIDC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " IDC, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zIDC Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zIDCpayAmount->setText("0");
    ui->labelzIDCSelected_int->setText("0");
...
void PrivacyDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zIDC </b>";
    ...
    CAmount idcBalance = balance - (immatureBalance + nLockedBalance);
    if (idcBalance < 0) idcBalance = 0;

    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zIDC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zIDC "));
    ui->labelzIDCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, idcBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zIDC </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zIDC </b> ";
        switch (denom) {
        case libzerocoin::CoinDenomination::ZQ_ONE:
...
void PrivacyDialog::showOutOfSyncWarning(bool fShow) {
    ui->labelzIDCSyncStatus->setVisible(fShow);
}
