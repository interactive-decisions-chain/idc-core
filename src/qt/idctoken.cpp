...
IDCToken::IDCToken(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::IDCToken),
    m_model(0),
    m_clientModel(0),
...
void IDCToken::setModel(WalletModel *_model)
{
    m_model = _model;
...
void IDCToken::setClientModel(ClientModel *_clientModel)
{
...
void IDCToken::on_goToSendTokenPage()
{
    m_sendAction->setChecked(true);
    ui->stackedWidget->setCurrentIndex(0);
}

void IDCToken::on_goToReceiveTokenPage()
{
    m_receiveAction->setChecked(true);
    ui->stackedWidget->setCurrentIndex(1);
}

void IDCToken::on_goToAddTokenPage()
{
    m_addTokenAction->setChecked(true);
    ui->stackedWidget->setCurrentIndex(2);
}

void IDCToken::on_currentTokenChanged(QModelIndex index)
{
    if(m_tokenModel)
    {
        if(index.isValid())
...
void IDCToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void IDCToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void IDCToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void IDCToken::contextualMenu(const QPoint &point)
{
...
void IDCToken::focusToken(const QModelIndex& idx)
{
    if (!m_tokenModel)
        return;
