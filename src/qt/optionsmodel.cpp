...
void OptionsModel::Init() {
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::IDC);
    ...
    if (!settings.contains("nAnonymizeIDChainAmount"))
        settings.setValue("nAnonymizeIDChainAmount", 1000);

    nAnonymizeIDChainAmount = settings.value("nAnonymizeIDChainAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeIDChainAmount"))
        SoftSetArg("-anonymizeidchainamount", settings.value("nAnonymizeIDChainAmount").toString().toStdString());
...
void OptionsModel::Reset() {
    ...
    settings.clear();
    resetSettings = true; // Needed in idchain.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const {
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIDChainAmount:
            return QVariant(nAnonymizeIDChainAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role) {
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIDChainAmount:
            nAnonymizeIDChainAmount = value.toInt();
            settings.setValue("nAnonymizeIDChainAmount", nAnonymizeIDChainAmount);
            emit anonymizeIDChainAmountChanged(nAnonymizeIDChainAmount);
            break;
        case CoinControlFeatures:
