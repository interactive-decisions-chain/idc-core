...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::IDCC);
    ...
    if (!settings.contains("nAnonymizeIDCCAmount"))
        settings.setValue("nAnonymizeIDCCAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeIDCCAmount = settings.value("nAnonymizeIDCCAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeIDCCAmount"))
        SoftSetArg("-anonymizeidccamount", settings.value("nAnonymizeIDCCAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in idcc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIDCCAmount:
            return QVariant(nAnonymizeIDCCAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case DarkSendRounds:
            ...
            break;
        case AnonymizeIDCCAmount:
            nAnonymizeIDCCAmount = value.toInt();
            settings.setValue("nAnonymizeIDCCAmount", nAnonymizeIDCCAmount);
            Q_EMIT anonymizeIDCCAmountChanged(nAnonymizeIDCCAmount);
            break;
        case CoinControlFeatures:
