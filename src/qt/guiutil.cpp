...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    // and this is the only place, where this address is supplied.
     widget->setPlaceholderText(QObject::tr("Enter a IDCC address (e.g. %1)").arg("IhfHsuK9Ekjj2dYoEy4wvEiQHvRtzZQnYa"));
#endif
...
{
    // return if URI is not valid or is no IDCC: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::IDCC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert idcc:// to idcc:
    //
    //    Cannot handle this later, because idcc:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::IDCC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open idcc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "IDCC.lnk";
}
...
{
    return GetAutostartDir() / "idcc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a idcc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=IDCC\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the idcc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add idcc app to startup item list
