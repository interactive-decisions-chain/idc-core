...
public:
    TxViewDelegate(const PlatformStyle *platformStyle):
            QAbstractItemDelegate(), unit(BitcoinUnits::IDCC),
                                              ...
                                              filter(0)
{
    ...
    ui->setupUi(this);
    //set Logo
    ui->labelLogo->setPixmap(QPixmap(":/images/idcc_logo_horizontal")
...
#else
       {
        if (fMasterNode) {
            ...
            if (!fEnableDarksend) {
                ...
                ui->toggleDarksend->setText(tr("Start IDCCsend"));
            } else {
                ui->darksendEnabled->setText(tr("Enabled"));
                ui->toggleDarksend->setText(tr("Stop IDCCsend"));
            }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("IDCC")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarkSendProgress()
{
    ...
    QString strAnonymizeIDCCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIDCCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (!fEnableDarksend) {
        ...
        strAnonymizeIDCCAmount = strAnonymizeIDCCAmount.remove(strAnonymizeIDCCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIDCCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeIDCCAmount * COIN) nMaxToAnonymize = nAnonymizeIDCCAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeIDCCAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeIDCCAmount));
        strAnonymizeIDCCAmount = strAnonymizeIDCCAmount.remove(strAnonymizeIDCCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIDCCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::darkSendStatus()
{
    ...
    if (strStatus != ui->darkSendStatus->text() && !strStatus.isEmpty()) {
        LogPrintf("Last IDCCsend message: %s\n", strStatus.toStdString().c_str());
