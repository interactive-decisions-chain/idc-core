// Copyright (c) 2018-2019 The IDCC Core Developer
                                                                             ...
                                                                             platformStyle(platformStyle)
{
    ...
    IDCTokenPage = new IDCToken(this);
    ...
    // Clicking on a token on the overview pre-selects the token on the IDC Token page
    connect(overviewPage, &OverviewPage::tokenClicked, IDCTokenPage, &IDCToken::focusToken);
...
void WalletView::setBitcoinGUI(BitcoinGUI* gui)
{
    if (gui) {
        ...
        // Clicking on a token on the overview page simply sends you to IDC Token page
        connect(overviewPage,SIGNAL(tokenClicked(QModelIndex)),
                gui, SLOT(gotoIDCTokenPage()));
        ...
        connect(overviewPage, SIGNAL(addTokenClicked(bool)), gui, SLOT(gotoIDCTokenPage(bool)));
...
void WalletView::setClientModel(ClientModel* clientModel)
{
    ...
    IDCTokenPage->setClientModel(clientModel);
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void WalletView::setWalletModel(WalletModel* walletModel)
{
    ...
    IDCTokenPage->setModel(walletModel);
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void WalletView::gotoIDCTokenPage(bool toAddTokenPage)
{
    setCurrentWidget(IDCTokenPage);
    if(toAddTokenPage)
        IDCTokenPage->on_goToAddTokenPage();
}
