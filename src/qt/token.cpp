...
bool ToHash160(const std::string& strIDCCAddress, std::string& strHash160)
{
    CTxDestination idccAddress = DecodeDestination(strIDCCAddress);
    if(IsValidDestination(idccAddress)){
        CKeyID *keyid = boost::get<CKeyID>(&idccAddress);
...
bool ToIDCCAddress(const std::string& strHash160, std::string& strIDCCAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);

    if(IsValidDestination(CTxDestination(keyid))){
        strIDCCAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToIDCCAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToIDCCAddress(tokenEvent.receiver, tokenEvent.receiver);
