...

{
    ...
    resize(1200, 750);
    QString windowTitle = tr("IDCCcore") + " - ";

#ifdef ENABLE_UPDATER
    controller = new QtIDCCUpdater::UpdateController(QStringLiteral("v5.3.0"), this);
    ...
    {
        QString url = "https://github.com/IDCC-Core/idcc/releases";
        QString link = QString("<a href=\\\"\"+ url +\"\\\">\"+ url +\"</a>").arg(NEW_RELEASES, NEW_RELEASES);
        QString message(tr("New idcc-qt version available: <br /> %1. <br />").arg(link));
...
void BitcoinGUI::createActions() {
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a IDCC address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and idcc: URIs)"));
    ...
    IDCTokenAction = new QAction(QIcon(":/icons/IDCtoken"), tr("&IDC Token"), this);
    IDCTokenAction->setStatusTip(tr("IDC Token (send, receive or add Token in list)"));
    IDCTokenAction->setToolTip(IDCTokenAction->statusTip());
    IDCTokenAction->setCheckable(true);
#ifdef Q_OS_MAC
    IDCTokenAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_5));
#else
    IDCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
#endif
    tabGroup->addAction(IDCTokenAction);
    ...
    connect(IDCTokenAction, SIGNAL(triggered()), this, SLOT(gotoIDCTokenPage()));
    ...
    aboutAction = new QAction(QIcon(":/icons/idcccoin_black"), tr("&About IDCCcore"), this);
    aboutAction->setStatusTip(tr("Show information about IDCCcore"));
    aboutAction->setMenuRole(QAction::AboutRole);

#ifdef ENABLE_UPDATER
    // Check for update menu item
    checkForUpdateAction = new QAction(QIcon(":/icons/update_black"), tr("Check for &Update"), this);
    checkForUpdateAction->setStatusTip(tr("Check whether there is an updated wallet from IDCCcore"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for IDCC"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/idcccoin_black"), tr("&Show / Hide"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your IDCC addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/verified"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified IDCC addresses"));
    ...
    openAction->setStatusTip(tr("Open a IDCC: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    openHexAddressAction = new QAction(QIcon(":/icons/editcopy"), tr("&Hex Address Converter"), this);
    openHexAddressAction->setStatusTip(tr("Converter for IDCC Smart Contract addresses"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the IDCCcore help message to get a list with possible IDCC command-line options"));
...
void BitcoinGUI::createToolBars() {
    if (walletFrame) {
        ...
        toolbar->addAction(IDCTokenAction);
        QSettings settings;
        if (settings.value("fShowMasternodesTab").toBool()) {
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle) {
    ...
    QString toolTip = tr("IDCCcore client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::updaterClicked() {
    if (!clientModel)
        return;
#ifdef ENABLE_UPDATER
    controller->start(QtIDCCUpdater::UpdateController::ProgressLevel);
#else
...
void BitcoinGUI::gotoIDCTokenPage(bool toAddTokenPage)
{
    IDCTokenAction->setChecked(true);
    if (walletFrame) walletFrame->gotoIDCTokenPage(toAddTokenPage);
}
...
void BitcoinGUI::updateNetworkState() {
    ...
    if (clientModel->getNetworkActive())
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to IDCCcore network", "", count));
    else
...
void BitcoinGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret) {
    QString strTitle = tr("IDCCcore"); // default title
