...
static void NotifyIDCCNodeUpdated(MasternodeManager *page, CIDCCNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateIDCCNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyIDCCNodeChanged.connect(boost::bind(&NotifyIDCCNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyIDCCNodeChanged.disconnect(boost::bind(&NotifyIDCCNodeUpdated, this, _1));
}
...
void MasternodeManager::updateIDCCNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        for (PAIRTYPE(std::string, CIDCCNodeConfig) adrenaline : pwalletMain->mapMyIDCCNodes)
        {
            updateIDCCNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditIDCCNode* aenode = new AddEditIDCCNode();
    aenode->exec();
}
#if 0
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CIDCCNodeConfig c = pwalletMain->mapMyIDCCNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    IDCCNodeConfigDialog* d = new IDCCNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyIDCCNodes.erase(sAddress);
        walletdb.EraseIDCCNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    for (PAIRTYPE(std::string, CIDCCNodeConfig) adrenaline : pwalletMain->mapMyIDCCNodes)
    {
        CIDCCNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
