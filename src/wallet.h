...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 IDC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zIDC send
enum ZerocoinSpendStatus {
    ZIDC_SPEND_OKAY = 0,                            // No error
    ZIDC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZIDC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZIDC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZIDC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZIDC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZIDC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZIDC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZIDC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZIDC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZIDC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZIDC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZIDC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZIDC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZIDC_SPENT_USED_ZIDC = 14,                      // Coin has already been spend
    ZIDC_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
class CWallet : public CCryptoKeyStore, public CValidationInterface {
  ...
  public:
    ...
    */
    ...
    void setZIDCAutoBackups(bool fEnabled) {
