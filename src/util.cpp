...
{
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // IDCC
    assert(mutexDebugLog == NULL);

    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // IDCC
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // IDCC
    if (fileout) setbuf(fileout, NULL); // unbuffered
    if (fileoutVM) setbuf(fileoutVM, NULL); // unbuffered // IDCC
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "idcc" is a composite category enabling all IDCC-related debug output
            if (ptrCategory->count(string("idcc"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\IDCC
// Windows >= Vista: C:\Users\Username\AppData\Roaming\IDCC
// Mac: ~/Library/Application Support/IDCC
// Unix: ~/.idcc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IDCC";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "idcc.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty idcc.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override idcc.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "idccd.pid"));
