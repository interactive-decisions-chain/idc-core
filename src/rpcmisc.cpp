...
// Copyright (c) 2015-2018 The IDCC developers
...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total idcc balance of the wallet\n"
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized idcc balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in idcc/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in idcc/kB\n"
...
UniValue getstateinfo(const UniValue& params, bool fHelp)
{
    ...
    IDCCDGP idccDGP(globalState.get());

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("maxblocksize", (int)idccDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (int)idccDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (int)idccDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"idccaddress\"\n"
            "\nReturn information about the given idcc address.\n"
            "\nArguments:\n"
            "1. \"idccaddress\"     (string, required) The idcc address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"idccaddress\",     (string) The idcc address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: IDCC address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are idcc addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) idcc address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"idccaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"idccaddress\"  (string, required) The idcc address to use for the signature.\n"
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "}\n"
            "\nResult: {\n"
            "  \"balance\",  (number) The current balance (in IDCC satoshis)\n"
